...
当用户说"继续开发"、"continue development"、"继续昨天的任务"时：

1. 读取 `AGENTS.md`
2. 读取 `docs/handoff.md`
3. 读取 `docs/tasks.md`
4. 定位当前任务文件
...
当用户问"当前进行到哪一步"、"进度汇报"、"what is next"时，汇报：
- 当前发布/阶段
- 当前任务
- 阻塞项（如有）
- 验收/验证状态
...
3. 维护 `docs/handoff.md` 作为每次工作结束时的交接记录
...
# 4. 代理会自动读取 handoff.md 和 tasks.md
...
│    └── ...                                   │
│                                              │
│  Chat  = Temporary context (lost per session) │
...
├── handoff.md                   # Latest handoff state (next agent's entry)
└── decisions/
...
When the user says "continue development", "继续开发", "continue yesterday's task":

1. Read `AGENTS.md`
2. Read `docs/handoff.md`
3. Read `docs/tasks.md`
4. Locate the current task file
...
- **Later release/phase**: valuable but not required for the current finish line
...
3. Maintain `docs/handoff.md` as the handoff record at the end of each work session
...
# 4. The agent automatically reads handoff.md and tasks.md,
